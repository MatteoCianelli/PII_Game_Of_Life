    ...
    public class Game
    {
        public static void NextGen(Board currentBoard)
        {
            ...
            for (int x = 0; x < boardWidth; x++)
            {
                for (int y = 0; y < boardHeight; y++)
                {
                    ...
                    for (int i = x - 1; i <= x + 1; i++)
                    {
                        ...
                        {
                            if (i >= 0 && i < boardWidth && j >= 0 && j < boardHeight && currentBoard.CellIsAlive(i, j))
                            {
                                aliveNeighbors++;
                            }
                        }
                    ...
                    if (currentBoard.CellIsAlive(x, y))
                    {
                        aliveNeighbors--;
                    }

                    if (currentBoard.CellIsAlive(x, y) && aliveNeighbors < 2)
                    {
                        // Célula muere por baja población
                        cloneboard[x, y] = false;
                    }
                    else if (currentBoard.CellIsAlive(x, y) && aliveNeighbors > 3)
                    {
                    ...
                    else
                    {
                        ...
                        cloneboard[x, y] = currentBoard.CellIsAlive(x, y);
                    }
                }
            }
            ...
            for (int x = 0; x < boardWidth; x++)
            {
                for (int y = 0; y < boardHeight; y++)
                {
                    currentBoard.ChangeCellValue(x, y, cloneboard[x, y]);
                }
            }
        }
    }
